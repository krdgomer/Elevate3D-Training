...
def example_usage():
    ...
    try:
        # Load input image
        input_img = cv2.imread("input4.png")
        if input_img is None:
            print("Error: Could not load input3.png")
            return None, None
            
        input_img = cv2.cvtColor(input_img, cv2.COLOR_BGR2GRAY)
        
        # Get DSM from your model
        print("Generating DSM...")
        dsm_uint8 = predict_dsm(input_img, model_path="src/dsm_generation/models/v0.7/weights/gen.pth.tar")
        ...
        return dtm_csf, float_to_dsm_uint8(height_csf)
